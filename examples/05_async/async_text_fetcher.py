...
class AsyncSefariaClient:
    ...
    async def get_text(
        self, session: aiohttp.ClientSession, tref: str, version: str = None
    ) -> Dict:
        ...
        async with session.get(
            f"{self.base_url}/v3/texts/{quote(tref)}", params=params
        ) as response:
            return await response.json()

    async def bulk_fetch_texts(
        self, refs: List[str], version: str = None
    ) -> List[Dict]:
        """Fetch multiple texts concurrently."""
        async with aiohttp.ClientSession() as session:
            tasks = [self.get_text(session, ref, version) for ref in refs]
...
async def main():
    ...
    # Print results
    print("\nResults:")
    for i, (ref, en, he) in enumerate(zip(texts_to_fetch, english_texts, hebrew_texts)):
        print(f"\n{ref}:")
