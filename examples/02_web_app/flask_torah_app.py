...
def weekly_portion():
    try:
        ...
        parsha = next(
            (item for item in calendar.get('calendar_items', [])
        ...
        english_response = client.get_text(ref, version="The Holy Scriptures: A New Translation (JPS 1917)")
        ...
        verses = [
            {'hebrew': h, 'english': e}
            for h, e in zip(hebrew_text, english_text)
        ] if hebrew_text and english_text else []
