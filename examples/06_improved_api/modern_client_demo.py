...
def demo_parasha_outline():
    ...
    try:
        ...
        opening_verse = (
            f"{parasha_ref.split('-')[0]} 1:1"  # First verse of first chapter
        )
            ...
            if "versions" in text:
                for version in text["versions"]:
                    ...
                    if lang in ["he", "en"] and version.get("text"):
                        print(
                            f"  {lang.upper()}: {version['text'][0] if version['text'] else 'N/A'}"
                        )
        except Exception as e:
        ...
        try:
            ...
            commentary_refs = [
                link["ref"] for link in links if link.get("type") == "commentary"
            ][
                :3
            ]  # First 3 commentaries
