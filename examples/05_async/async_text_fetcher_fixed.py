...
class AsyncSefariaClient:
    ...
    async def get_text(self, session: aiohttp.ClientSession, 
                      tref: str, version_title: str = None, lang: str = None) -> Dict:
    ...
    async def bulk_fetch_texts(self, refs: List[str], version_title: str = None, lang: str = None) -> List[Dict]:
        """Fetch multiple texts concurrently."""
        async with aiohttp.ClientSession() as session:
            tasks = [
                self.get_text(session, ref, version_title, lang)
                for ref in refs
            ]
            return await asyncio.gather(*tasks)
            
    async def get_english_version_title(self, session: aiohttp.ClientSession, book: str) -> str:
        """Get the first available English version title for a book."""
        async with session.get(f"{self.base_url}/texts/versions/{quote(book)}") as response:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        version_tasks = [client.get_english_version_title(session, book) for book in books]
    ...
    async with aiohttp.ClientSession() as session:
        ...
        english_texts, hebrew_texts = await asyncio.gather(
            asyncio.gather(*english_tasks),
            asyncio.gather(*hebrew_tasks)
        )
    
    # Print results
    print("\nResults:")
    for i, (ref, en_data, he_data) in enumerate(zip(texts_to_fetch, english_texts, hebrew_texts)):
        print(f"\n{ref}:")
