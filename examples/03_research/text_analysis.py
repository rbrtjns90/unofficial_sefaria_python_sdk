...
class TextAnalyzer:
    ...
    def analyze_word_frequency(self, text_ref: str, language: str = 'en') -> Dict[str, int]:
    ...
    def find_common_links(self, text_ref: str) -> List[Tuple[str, int]]:
        ...
        try:
            ...
            if isinstance(links, list):
                ...
                refs = [link.get('ref', '') for link in links['links'] if link.get('ref')]
...
def main():
    ...
    if freq_en:
        ...
        for word, count in sorted(freq_en.items(), key=lambda x: x[1], reverse=True)[:10]:
            print(f"  {word}: {count}")
    else:
        ...
        if freq_he:
            for word, count in sorted(freq_he.items(), key=lambda x: x[1], reverse=True)[:5]:
                print(f"  {word}: {count}")
